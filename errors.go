...
func (e ErrDefault502) Error() string {
	e.DefaultErrString = `Bad Gateway. Try to access the website and the proxy server cannot get a valid response.
It may be caused by the upstream server being temporarily unavailable, timing out, or returning a malformed response.`
	return e.choseErrString()
}
