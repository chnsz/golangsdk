...
func buildReqWithTestcase(tc TestCase) (*http.Request, error) {
	...
	urlPath := strings.TrimPrefix(tc.Path, "/")
	baseURL.Path, err = CustomJoinPath(baseURL.Path, urlPath)
	if err != nil {
		return nil, err
	}
...
func CustomJoinPath(base string, elem ...string) (string, error) {
	parsedURL, err := url.Parse(base)
	if err != nil {
		return "", err
	}

	newPath := gosrcPath.Join(append([]string{parsedURL.Path}, elem...)...)
